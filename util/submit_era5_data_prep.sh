...
export STD_OUT_PATH=${USER_LOG_DIR}/era5_data_prep_log_$(date '+%Y%m%d%H%M').out
export STD_ERR_PATH=${USER_LOG_DIR}/era5_data_prep_log_$(date '+%Y%m%d%H%M').err
export JOBNAME=era5_ae_dataprep_$(date '+%Y%m%d%H%M')

sbatch -o $STD_OUT_PATH -e $STD_ERR_PATH -J ${JOBNAME} --export=ALL,USER_DIR,AI4CLIMATE_GWS_DIR,AI4C_HACKATHON_REPO_DIR util/run_era5_data_prep.sh 
